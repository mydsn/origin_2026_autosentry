...
static void Chassis_Max_Power_Update(fp32 *chassis_max_power) // 根据不同模式选择不同底盘功率上限
{
	...
	if (cap_data.cap_per > 0.2f) // 超电还有电，可以用超电达到超越裁判系统功率上限的底盘功率
	{
		...
		case NAV_NORMAL_MODE:
			...
			if (nav_ctrl.game_start)
			{
				...
				*chassis_max_power = limit(nav_ctrl.referee_power_limit * power_factor, 0.0f, 150.0f);
			}
			else
			{
...
void Chassis_Task(void const *argument)
{
	...
	while (1)
	{
		...
		// Vofa_Send_Data4(real_power, power_limiter.chassis_power_predicted, power_limiter.chassis_power_processed, power_limiter.iter_num);
