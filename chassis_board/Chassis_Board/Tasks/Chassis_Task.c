...
typedef struct // 底盘控制参数结构体
{
	...
	bool_t is_intended_stop_on_bumpy;		   // 是否想要停车，用于在颠簸路段上防止进入自锁状态
	bool_t need_restore_buffer_energy; // 是否需要恢复缓冲能量
	bool_t bumpy_force_safe;		   // 是否处于颠簸路段复活时的强制失能状态
	uint32_t bumpy_revive_power_timer; // 颠簸路段复活时的功率放大计时器
	bool_t waiting_for_wz_slow_down;   // 是否正在等待小陀螺减速到底盘跟随模式阈值
...
chassis_control_t chassis_control = {
	...
	.is_stop = TRUE,
	.is_stop_last = TRUE,
	.bumpy_force_safe = FALSE,
	.bumpy_revive_power_timer = 0,
};
// 功率控制结构体初始化
power_limiter_t power_limiter =
	{
		.wheel_motors =
			{
				.k_p = M3508_K_P,
...
static void Chassis_Mode_Update(chassis_mode_t *mode)
{
	static uint8_t last_just_revive = 0;

	if (nav_ctrl.just_revive == 1 && last_just_revive == 0) // 死在颠簸路段上，复活的时候判断一下有没有能力脱困，不行就等超电充会电
	{
		if (chassis_control.chassis_max_power_mode == PASS_BUMPY && nav_ctrl.referee_power_limit < PASS_BUMPY_POWER_THRESHOLD && cap_data.cap_per < (CAP_USE_THRESHOLD + REVIVE_ON_BUMPY_CAP_EXTRA_THRESHOLD))
		{
			chassis_control.bumpy_force_safe = TRUE;
		}
	}

	if (chassis_control.bumpy_force_safe)
	{
		if (nav_ctrl.referee_power_limit >= PASS_BUMPY_POWER_THRESHOLD || (cap_data.cap_per >= (CAP_USE_THRESHOLD + REVIVE_ON_BUMPY_CAP_EXTRA_THRESHOLD) && (!toe_is_error(CAP_TOE))))
		{
			chassis_control.bumpy_force_safe = FALSE;
		}
	}

	last_just_revive = nav_ctrl.just_revive;
	...
	{
		if (my_fabsf(chassis_control.current_wz) < ROTATE_SLOW_DOWN_THRESHOLD) // 减速到阈值以下才允许切换
		{
...
static void Chassis_Max_Power_Update(fp32 *chassis_max_power) // 根据不同模式选择不同底盘功率上限
{
	...
	if (toe_is_error(CAP_TOE)) // 无法与超电通讯，用裁判系统功率上限的85%作为底盘功率上限
	{
#if HAVE_REFEREE_SYSTEM
	...
	if (cap_data.cap_per > CAP_USE_THRESHOLD) // 超电还有电，可以用超电达到超越裁判系统功率上限的底盘功率
	{
		...
		case PASS_BUMPY:
#if HAVE_REFEREE_SYSTEM
			if (nav_ctrl.just_revive == 1) // 如果刚刚在颠簸路段复活
			{
				chassis_control.bumpy_revive_power_timer = xTaskGetTickCount(); // 记录或更新时间戳
			}

			if (chassis_control.bumpy_force_safe)
			{
				// 如果处于force_safe，不进行大功率输出，将计时器刷新，确保一旦退出force_safe能立刻开启3秒的大功率
				chassis_control.bumpy_revive_power_timer = xTaskGetTickCount();
				*chassis_max_power = (nav_ctrl.referee_power_limit * 0.9f) > 120.0f ? (nav_ctrl.referee_power_limit * 0.9f) : 120.0f;
			}
			else
			{
				if (xTaskGetTickCount() - chassis_control.bumpy_revive_power_timer <= 3000)
				{
					// 在刚复活或刚退出force_safe的3秒内，将功率推到200瓦
					*chassis_max_power = 200.0f;
				}
				else
				{
					// 3秒过后，恢复普通颠簸路段逻辑
		...
		case UPDOWN_HILL:
#if HAVE_REFEREE_SYSTEM
			*chassis_max_power = limit(nav_ctrl.referee_power_limit * 0.9f + cap_data.cap_per * 80, 50.0f, 150.0f);
#else
			*chassis_max_power = 120;
#endif
			break;
		...
		case HURT:
#if HAVE_REFEREE_SYSTEM
			*chassis_max_power = limit(nav_ctrl.referee_power_limit * 0.9f + cap_data.cap_per * 100, 32.0f, 170.0f);
#else
		...
		case NAV_NORMAL_MODE:
			...
			{
				fp32 time_progress = (420.0f - (fp32)nav_ctrl.game_remain_time) / 420.0f; // 0.0为开局，1.0为结束
				fp32 estimated_energy_level = 0.0f;

				switch (nav_ctrl.total_energy_remain)
				{
				case ABOVE_FIFTY_PERCENT:
					estimated_energy_level = 0.75f;
					break;
				case THIRTY_PERCENT_TO_FIFTY_PERCENT:
					estimated_energy_level = 0.40f;
					break;
				case FIVE_PERCENT_TO_THIRTY_PERCENT:
					estimated_energy_level = 0.175f;
					break;
				case BELOW_FIVE_PERCENT:
					estimated_energy_level = 0.025f;
					break;
				}

				fp32 expected_energy_remain = 1.0f - time_progress;					   // 期望的剩余能量百分比（线性消耗假设）
				fp32 energy_surplus = estimated_energy_level - expected_energy_remain; // 能量盈余（正表示省电，负表示耗电快）

				fp32 power_factor = 0.85f + energy_surplus;		// 根据盈余动态调整功率比例
				power_factor = limit(power_factor, 0.4f, 0.9f); // 限制最低40%，最高90%

				*chassis_max_power = limit(nav_ctrl.referee_power_limit * power_factor, 0.0f, 120.0f);
			}
			else
			{
	...
	else // 超电要没电了，只能用裁判系统功率上限，还要给超电充点电
	{
#if HAVE_REFEREE_SYSTEM
		if (chassis_control.chassis_max_power_mode == PASS_BUMPY && nav_ctrl.referee_power_limit > 85)
			*chassis_max_power = nav_ctrl.referee_power_limit * 0.9f;
		else if (nav_ctrl.referee_power_limit > 40.0f)
			*chassis_max_power = nav_ctrl.referee_power_limit * 0.9f - (1 - cap_data.cap_per) * 15;
		else
			*chassis_max_power = nav_ctrl.referee_power_limit * 0.85;
#else
		*chassis_max_power = 90;
#endif
	}
...
static void Set_Chassis_VxVy(fp32 yaw_chassis_zero_rad, fp32 *chassis_vx, fp32 *chassis_vy)
{
	...
	else // 导航模式（进入Set_FollowGimbal_VxVy函数时不是遥控器控制模式就是导航模式，所以不用再判断一次是否为导航模式）
	{
		if (!toe_is_error(NAV_TOE))
		{
			...
			// 刚进入过颠簸模式时判断功率和超电是否满足要求，不满足就停下，直到满足条件再走
			...
			{
				if (nav_ctrl.referee_power_limit < PASS_BUMPY_POWER_THRESHOLD && cap_data.cap_per < (CAP_USE_THRESHOLD + 0.4f))
				{
			...
			{
				if (my_fabsf(chassis_control.current_wz) < ROTATE_SLOW_DOWN_THRESHOLD) // 转速下降到阈值以下才能走
				{
	...
	{
		gimbal_vx = 100.0f; // 这个数值只会让舵朝前，轮电机的目标转速会在chassis_control.is_intended_stop_on_bumpy为true的时候置零
		gimbal_vy = 0.0f;
	}
	...
	else // 导航模式下的小陀螺速度设置
	{
		...
		else
		{
			...
			case NAV_ROTATE_UPHILL:
				target_wz = ROTATE_WZ_MIN * RAD_PER_SEC_TO_RPM; // 上坡状态下不转陀螺
				ramp_coeff = 0.2f;
				break;

			case NAV_ROTATE_HURT:
				// 被弹丸打了，根据底盘xy轴目标速度设置陀螺速度
	...
	for (uint8_t i = 0; i < 4; i++)
	{
		...
		if (my_fabsf(angle_error) > 15.0f && my_fabsf((float)chassis_steer_motor[i].speed_now) < 10.0f)
		{
			...
			if (steer_stuck_count[i] > STUCK_ENTER_COUNT) // 持续满足条件约 150次（时间依调度频率定）判定为卡死
			{
				...
				if (steer_stuck_count[i] > 1000)
					steer_stuck_count[i] = 1000;
			}
		}
		else
		{
...
static void Chassis_Motor_Control_Current_Set(chassis_mode_t mode)
{
	...
	for (uint8_t i = 0; i < 4; i++)
	{
		...
		{
			chassis_steer_motor[i].angle_pid.Kp = STEER_MOTOR_ANGLE_PID_KP;
			chassis_steer_motor[i].angle_pid.Ki = STEER_MOTOR_ANGLE_PID_KI;
			chassis_steer_motor[i].angle_pid.Kd = STEER_MOTOR_ANGLE_PID_KD;
		}
...
void Chassis_Task(void const *argument)
{
	...
	while (1)
	{
		...
		Check_Bumpy_Steer_Stuck(); // 更新颠簸路段卡舵状态
		...
		power_limiter.weight_allocate_mode = (chassis_control.chassis_max_power_mode == PASS_BUMPY) ? PASS_BUMPY_ALLOCATE : NORMAL_WEIGHT_ALLOCATE;
		Chassis_Power_Control(&power_limiter, chassis_wheel_motor, chassis_steer_motor, chassis_control.chassis_max_power, power_limiter.weight_allocate_mode, chassis_control.steer_stuck_status);
