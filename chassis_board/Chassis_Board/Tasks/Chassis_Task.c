...
typedef enum // 底盘功率上限枚举体，不同的值对应不同的底盘功率上限
{
	...
	UPDOWN_HILL,
	PASS_BUMPY,
	NAV_FIRST_GO_TO_MIDDLE, // rmul导航开局抢中
...
static void Chassis_Max_Power_Update(fp32 *chassis_max_power) // 根据不同模式选择不同底盘功率上限
{
	...
	case RC_SW_UP:
		...
		else if (nav_ctrl.rmul_first_go_to_middle && !nav_ctrl.at_middle_section) // 开局抢中
			chassis_control.chassis_max_power_mode = NAV_FIRST_GO_TO_MIDDLE;
...
#endif
			break;
		case NAV_FIRST_GO_TO_MIDDLE:
#if HAVE_REFEREE_SYSTEM
			*chassis_max_power = 130;
#else
...
#if HAVE_REFEREE_SYSTEM
			*chassis_max_power = limit(nav_ctrl.referee_power_limit * 0.95f, 0.0f, 100.0f);
#else
			*chassis_max_power = 80;
#endif
			break;
...
static void Set_Chassis_VxVy(fp32 yaw_chassis_zero_rad, fp32 *chassis_vx, fp32 *chassis_vy)
{
	...
	else // 导航模式（进入Set_FollowGimbal_VxVy函数时不是遥控器控制模式就是导航模式，所以不用再判断一次是否为导航模式）
	{
		if (!toe_is_error(NAV_TOE))
		{
			...
			fp32 ramp_coeff = 0.0f;
			if (nav_ctrl.at_middle_section)
				ramp_coeff = 0.15f;
			else if (nav_ctrl.updownhill_state == 2)
				ramp_coeff = 0.03f;
			else
...
static void Set_Rotate_Wz(fp32 *wz)
{
	...
	else // 导航模式下的小陀螺速度设置
	{
		...
		typedef enum
		{
			...
			NAV_ROTATE_FIRST_GO_MIDDLE, // 开局抢中
			NAV_ROTATE_ESCAPE,			// 被打逃跑
			NAV_ROTATE_MIDDLE_SECTION,	// 中央增益区
		...
		else
		{
			...
			if (nav_ctrl.updownhill_state == TRUE)
				...
				current_rotate_state = NAV_ROTATE_ESCAPE;
			else if (nav_ctrl.rmul_first_go_to_middle)
				current_rotate_state = NAV_ROTATE_FIRST_GO_MIDDLE;
			else if (nav_ctrl.at_middle_section)
				current_rotate_state = NAV_ROTATE_MIDDLE_SECTION;
			else
			...
			case NAV_ROTATE_FIRST_GO_MIDDLE:
				// 开局抢中
			...
			case NAV_ROTATE_MIDDLE_SECTION:
				// 在中央增益区，变速陀螺
				target_wz = (ROTATE_WZ_HIGH + 2.0f * arm_sin_f32((xTaskGetTickCount() % 1500) / 1000.0f * PI)) * RAD_PER_SEC_TO_RPM;
				ramp_coeff = 0.4f; // 变速陀螺斜坡
				break;
