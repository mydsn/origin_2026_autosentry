...
static void Chassis_Mode_Update(chassis_mode_t *mode)
{
	...
	{
		if (chassis_control.chassis_max_power_mode == PASS_BUMPY && nav_ctrl.referee_power_limit < 70 && cap_data.cap_per < (CAP_USE_THRESHOLD + REVIVE_ON_BUMPY_CAP_EXTRA_THRESHOLD))
		{
	...
	{
		if (nav_ctrl.referee_power_limit >= 70 || (cap_data.cap_per >= (CAP_USE_THRESHOLD + REVIVE_ON_BUMPY_CAP_EXTRA_THRESHOLD) && (!toe_is_error(CAP_TOE))))
		{
...
static void Chassis_Max_Power_Update(fp32 *chassis_max_power) // 根据不同模式选择不同底盘功率上限
{
	...
	if (cap_data.cap_per > CAP_USE_THRESHOLD) // 超电还有电，可以用超电达到超越裁判系统功率上限的底盘功率
	{
		...
		case PASS_BUMPY:
#if HAVE_REFEREE_SYSTEM
			if (nav_ctrl.just_revive == 1) // 刚刚在颠簸路段复活
			{
...
static void Set_Chassis_VxVy(fp32 yaw_chassis_zero_rad, fp32 *chassis_vx, fp32 *chassis_vy)
{
	...
	else // 导航模式（进入Set_FollowGimbal_VxVy函数时不是遥控器控制模式就是导航模式，所以不用再判断一次是否为导航模式）
	{
		if (!toe_is_error(NAV_TOE))
		{
			...
			{
				if (nav_ctrl.referee_power_limit < 70 && cap_data.cap_per < (CAP_USE_THRESHOLD + 0.4f))
				{
